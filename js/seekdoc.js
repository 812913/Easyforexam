...
//            表单失去焦点或提交时，进行验证
    ...
    $('#seekbtn>input').click(function (e) {
        ...
        if ($('#seekinfo>li:first-child>input').val() != '' && $('#seekinfo>li:nth-child(3)>select>option:selected').val() != '学院' && $('#seekinfo>li:nth-child(4)>select>option:selected').val() != '年级' && $('#seekinfo>li:nth-child(5)>select>option:selected').val() != '期限' && $('#seekinfo>li:nth-child(5)>select>option:selected').val() != '下载卷' && $('#seekinfo>li:last-child>input[type="text"]').val() == $('#seekinfo>li:last-child>input[type="button"]').val()) {
            e.stopPropagation();
        }
